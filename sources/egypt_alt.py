...
def _fetch_linkedin_egypt_search():
    jobs = []
    seen = set()
    budget = LINKEDIN_SOURCE_BUDGET_SECONDS
    t0 = time.time()
    for kw in EGYPT_SEARCH_KEYWORDS:
        if time.time() - t0 > budget:
            log.info("egypt_alt/search: 60s budget hit � stopping early")
            break
        params = (
...
def _fetch_linkedin_eg_private():
    ...
    for company_name, slug in LINKEDIN_EG_PRIVATE:
        if time.time() - t0 > budget:
            log.info("egypt_alt/private: 60s budget hit � stopping early")
            break
...
def _fetch_linkedin_eg_gov():
    ...
    for company_name, slug in LINKEDIN_EG_GOV:
        if time.time() - t0 > budget:
            log.info("egypt_alt/gov: 60s budget hit � stopping early")
            break
        url = f"{JOBS_API}?keywords=security&f_C={slug}&start=0&count=10"
        html = get_text(url, headers=_H)
        if not html:
...
def _fetch_linkedin_by_governorate():
    ...
    for gov in TOP_GOVERNORATES:
        for kw in GOVERNORATE_KEYWORDS:
            if time.time() - t0 > budget:
                log.info("egypt_alt/governorate: 90s budget hit � stopping early")
                break
            params = (
                f"?keywords={urllib.parse.quote(kw)}"
...
def _fetch_wuzzuf():
    jobs = []
    seen = set()
    budget = 60  # v37: raised 3060s for RSS (faster than HTML, more queries)
    t0 = time.time()

    for q in WUZZUF_RSS_QUERIES:
        if time.time() - t0 > budget:
            log.info("egypt_alt/wuzzuf: budget hit � stopping early")
            break
...
def fetch_egypt_alt() -> list:
    ...
    v42: Removed Wuzzuf � HTTP 404 on all endpoints (permanently dead).
    """
    all_jobs = []
    t0 = time.time()
    for fn in [
        _fetch_linkedin_egypt_search,    # 90s budget
        # _fetch_linkedin_eg_private,    # disabled: slow private sweep returned 2 jobs/120s in logs
        _fetch_linkedin_eg_gov,          # 60s budget
        _fetch_linkedin_by_governorate,  # 60s budget (trimmed)
        # _fetch_wuzzuf � REMOVED: HTTP 404 on all endpoints, wastes 30s every run
    ]:
        if time.time() - t0 > LINKEDIN_SOURCE_BUDGET_SECONDS:
            log.info("egypt_alt: 60s total budget hit � stopping remaining sub-sources")
            break
        try:
            all_jobs.extend(fn())
