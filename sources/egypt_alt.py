...
BAYT_QUERIES = [
    ...
    "SOC analyst", "network security", "security engineer",
]

def _fetch_bayt_egypt():
    jobs = []
    seen = set()
    for q in BAYT_QUERIES:
        url  = f"https://www.bayt.com/en/egypt/jobs/{urllib.parse.quote(q)}-jobs/"
        html = get_text(url, headers=_H)
        if not html:
            continue
        for block in re.findall(
            ...
            try:
                ...
                for item in items:
                    ...
                    jobs.append(Job(
                        title=title, company=company, location="Egypt",
                        url=item.get("url", url),
                        source="bayt", tags=["bayt", "egypt"],
                    ))
            except Exception:
                continue
        time.sleep(0.4)
    log.info(f"Bayt Egypt: {len(jobs)} jobs")
    return jobs


# ─── 6. Naukrigulf Egypt ─────────────────────────────────────
def _fetch_naukrigulf_egypt():
    jobs = []
    seen = set()
    queries = [
    ...
    for q in queries:
        url  = f"https://www.naukrigulf.com/{urllib.parse.quote(q.replace(' ','-'))}-jobs-in-egypt"
        html = get_text(url, headers=_H)
        if not html:
            continue
        for block in re.findall(
            ...
            try:
                ...
                for item in items:
                    ...
                    jobs.append(Job(
                        title=title, company=company, location="Egypt",
                        url=item.get("url", url),
                        source="naukrigulf", tags=["naukrigulf", "egypt"],
                    ))
            except Exception:
                continue
        time.sleep(0.4)
...
# ─── 7. Forasna.com RSS ──────────────────────────────────────
FORASNA_FEEDS = [
    "https://www.forasna.com/rss/technology",
    "https://www.forasna.com/en/technology-jobs-in-egypt/rss",
]

def _fetch_forasna():
    jobs = []
    seen = set()
    for url in FORASNA_FEEDS:
        ...
        try:
            ...
            for item in root.findall(".//item"):
                ...
                seen.add(link)
                jobs.append(Job(
                    title=title, company="Forasna",
                    location="Egypt", url=link,
                    source="forasna", tags=["forasna", "egypt"],
                ))
        except ET.ParseError:
            continue
    log.info(f"Forasna Egypt: {len(jobs)} jobs")
    return jobs
...
def _fetch_tanqeeb():
    jobs = []
    seen = set()
    for q in ["cybersecurity", "information security", "security engineer"]:
        url  = f"https://tanqeeb.com/en/jobs?q={urllib.parse.quote(q)}&country=EG"
        html = get_text(url, headers=_H)
        if not html:
            continue
        for block in re.findall(
            ...
            try:
                ...
                continue
        time.sleep(0.3)
    log.info(f"Tanqeeb Egypt: {len(jobs)} jobs")
    return jobs


def fetch_egypt_alt():
    """Aggregate Egypt alternative sources — zero-warning edition."""
    all_jobs = []
    for fetcher in [
