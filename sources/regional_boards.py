...
def _direct_get_text(url: str, timeout: int = 10, retries: int = 2) -> str | None:
    """Compatibility wrapper over the shared, rate-limited client.

    These specific boards have historically rejected requests routed through
    the proxy pool (proxy-auth failures show up as spurious 403/407s), so we
    force a direct connection here via use_proxy=False. This still goes
    through the shared session/retry/rate-limit/metrics logic — unlike a
    hand-rolled second `requests.Session()`, failures here are still counted
    and retried consistently with the rest of the bot.
    """
    return get_text(url, headers=_H, timeout=timeout, max_retries=retries, use_proxy=False)
