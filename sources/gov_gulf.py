...
def _fetch_gulf_linkedin_companies():
    ...
    for company_name, slug in LINKEDIN_GULF_COMPANIES:
        if _t.time() - t0 > budget:
            log.info("gov_gulf/companies: budget hit � stopping early")
            break
    ...
    for loc in GULF_LOCATIONS:  # all 6 countries
        for kw in GULF_KEYWORDS[:3]:  # top 3 keywords per country
            if time.time() - t0 > LINKEDIN_SOURCE_BUDGET_SECONDS:
                log.info("gov_gulf/search: 60s budget hit � stopping early")
                return jobs
            params = (
                f"?keywords={urllib.parse.quote(kw)}"
                f"&location={urllib.parse.quote(loc)}"
                "&start=0&count=5&f_TPR=r172800"
            )
...
def fetch_gov_gulf():
    ...
    v42: Removed _fetch_gulf_linkedin_companies � consistently 0 jobs every run.
    """
    all_jobs = []
    t0 = time.time()
    for fetcher in [
        ...
        _fetch_etisalat_uae,
        # _fetch_gulf_linkedin_companies,  # REMOVED: 0 jobs consistently
        _fetch_linkedin_gulf_search,
    ]:
        if time.time() - t0 > LINKEDIN_SOURCE_BUDGET_SECONDS:
            log.info("gov_gulf: 60s total budget hit � stopping remaining sub-sources")
            break
        try:
