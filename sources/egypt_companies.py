...
def _fetch_linkedin_eg_security_companies():
    ...
    budget = 240  # v37: raised 120240s � budget was hitting early due to retry sleep time not counted
    t0 = _time2.time()
    for company_name, slug in LINKEDIN_EG_SECURITY_COMPANIES:
        if _time2.time() - t0 > budget:
            log.info("egypt_companies/linkedin: 240s budget hit � stopping early")
            break
        for kw in _COMPANY_KW[:2]:  # 2 keywords max per company to stay under rate limits
            ...
            for i, title in enumerate(titles):
                ...
                _eg = ["egypt", "cairo", "alexandria", "giza", ""]
...
def _fetch_egypt_internships():
    ...
    for kw in INTERNSHIP_KEYWORDS[:5]:
        params = (
            ...
            "&location=Egypt&start=0&count=10&f_TPR=r172800"
    ...
    # Wuzzuf internships removed: endpoints returned HTTP 404 in production logs.
...
def fetch_egypt_companies():
    ...
    all_jobs = []
    for fetcher in [
        # _fetch_linkedin_eg_security_companies,  # disabled: 486s for 2 jobs in logs
