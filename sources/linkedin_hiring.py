...
BUDGET_SECS = LINKEDIN_SOURCE_BUDGET_SECONDS
...
def _parse_detail(html: str, job_id: str, search_location: str = "") -> Job | None:
    ...
    salary = extract_salary_from_text(f"{raw_title} {description}")
...
def _parse_from_list_html(html: str, job_id: str, search_location: str = "") -> Job | None:
    ...
    salary = extract_salary_from_text(raw_title)
    posted_date = None
    age_match = re.search(
        r"(\d{1,2})\s*(minute|minutes|min|hour|hours|hr|day|days|d|week|weeks|w)\s+ago",
        block,
...
def fetch_linkedin_hiring() -> list[Job]:
    ...
    for search in ALL_HIRING_SEARCHES:
        if time.time() - start_time > BUDGET_SECS:
            log.info(f"LinkedIn #Hiring: budget exhausted after {len(jobs)} jobs.")
            break
        if total_failures >= MAX_TOTAL:
            log.warning("LinkedIn #Hiring: max failures reached � stopping.")
            break
        if consecutive_failures >= MAX_CONSECUTIVE:
            log.warning(f"LinkedIn #Hiring: {consecutive_failures} consecutive failures � stopping this source.")
            break
