...
from run_budget import cap_source_timeout, cap_timeout, remaining, source_remaining
...
_metrics = {
    "requests": 0, "429": 0, "403": 0, "402": 0, "407": 0,
    "timeouts": 0, "errors": 0, "direct_bypass": 0,
    "circuit_open": 0, "endpoint_circuit_open": 0,
}
_run_circuit_lock = threading.Lock()
_proxy_provider_open = False
_endpoint_circuits: set[str] = set()
...
def reset_http_run_state() -> None:
    """Reset only in-memory circuits and per-run telemetry at run start."""
    global _proxy_provider_open
    with _run_circuit_lock:
        _proxy_provider_open = False
        _endpoint_circuits.clear()
    with _metrics_lock:
        for key in _metrics:
            _metrics[key] = 0
...
def _open_endpoint_circuit(url: str) -> None:
    key = _endpoint_key(url)
    with _run_circuit_lock:
        if key not in _endpoint_circuits:
            _endpoint_circuits.add(key)
            with _metrics_lock:
                _metrics["endpoint_circuit_open"] += 1


def _open_proxy_provider_circuit() -> None:
    global _proxy_provider_open
    with _run_circuit_lock:
        if not _proxy_provider_open:
            _proxy_provider_open = True
            with _metrics_lock:
                _metrics["circuit_open"] += 1


def _proxy_provider_is_open() -> bool:
    with _run_circuit_lock:
        return _proxy_provider_open
...
def _request_with_retry(
    ...
    *,
    ...
    max_retries: int = 2,
    use_proxy: bool = True,
    budget_phase: str | None = "other_sources",
) -> requests.Response | None:
    """Make one bounded proxy attempt and, when needed, one direct fallback.

    ``max_retries`` remains in the public signature for connector compatibility,
    but intentionally no longer causes the same endpoint to be hammered.  A
    failed proxy route may fall back to direct once; terminal failure opens an
    in-run endpoint circuit.
    """
    ...
    with _run_circuit_lock:
        if endpoint in _endpoint_circuits:
            _thread_local.last_http_error = "endpoint_circuit_open"
            return None
    if source_remaining() <= 0:
        _thread_local.last_http_error = "source_budget_exhausted"
        return None
    if remaining(budget_phase) <= 0:
        _thread_local.last_http_error = "run_budget_exhausted"
        return None

    domain = _domain(url)
    proxy: dict[str, str] | None = None
    if use_proxy and _proxy_pool.enabled and not _is_linkedin_url(url) and not _proxy_provider_is_open():
        proxy = _proxy_pool.get(host=domain)

    # ``None`` explicitly means a direct request.  The ordered attempt chain
    # is max two entries by design: proxy (optional), then direct fallback.
    routes: list[dict[str, str] | None] = [proxy] if proxy else [None]
    if proxy:
        routes.append(None)

    for route_index, route_proxy in enumerate(routes):
        if source_remaining() <= 0:
            _thread_local.last_http_error = "source_budget_exhausted"
            _open_endpoint_circuit(url)
            return None
        if remaining(budget_phase) <= 0:
            _thread_local.last_http_error = "run_budget_exhausted"
            _open_endpoint_circuit(url)
        ...
        try:
            with _metrics_lock:
                _metrics["requests"] += 1
                if route_index and proxy:
                    _metrics["direct_bypass"] += 1
            response = session.request(
                ...
                json=json,
                timeout=min(
                    cap_timeout(timeout, phase=budget_phase),
                    cap_source_timeout(timeout),
                ),
                allow_redirects=True,
                proxies=route_proxy or {},
            )
        except requests.exceptions.ProxyError as exc:
            with _metrics_lock:
                _metrics["errors"] += 1
            proxy_url = _proxy_url_of(route_proxy)
            err = str(exc).lower()
            if "402" in err or "payment required" in err:
                with _metrics_lock:
                    _metrics["402"] += 1
                _open_proxy_provider_circuit()
            ...
            _thread_local.last_http_error = "proxy_402" if "402" in err else "proxy_error"
            if route_proxy is None:
                _open_endpoint_circuit(url)
                return None
            continue
        except requests.RequestException as exc:
            with _metrics_lock:
                ...
                if isinstance(exc, requests.Timeout):
                    _metrics["timeouts"] += 1
            _thread_local.last_http_error = type(exc).__name__.lower()
            if route_proxy is None:
                _open_endpoint_circuit(url)
                return None
            continue
        ...
        # A 402 while a proxy was in use is a proxy-provider/billing failure,
        # not evidence that the target itself is unavailable.
        if response.status_code == 402 and route_proxy:
            with _metrics_lock:
                _metrics["402"] += 1
            _open_proxy_provider_circuit()
            proxy_url = _proxy_url_of(route_proxy)
            if proxy_url:
                _proxy_pool.ban(proxy_url, reason="conn_error")
            _thread_local.last_http_error = "proxy_402"
            continue

        if response.status_code == 402:
            with _metrics_lock:
                _metrics["402"] += 1
            _thread_local.last_http_error = "http_402"
            _open_endpoint_circuit(url)
            return None
        ...
        if response.status_code == 429:
            ...
            if proxy_url:
                ...
                _thread_local.last_http_error = "http_429"
                continue
            _thread_local.last_http_error = "http_429"
            _open_endpoint_circuit(url)
            return None
        ...
        if response.status_code == 403:
            ...
            if proxy_url:
                ...
                _thread_local.last_http_error = "http_403"
                continue
            _thread_local.last_http_error = "http_403"
            _open_endpoint_circuit(url)
            return None
        ...
        if response.status_code == 407:
            ...
            if proxy_url:
                _proxy_pool.ban(proxy_url, reason="403")
                _thread_local.last_http_error = "proxy_407"
                continue
            _thread_local.last_http_error = "http_407"
            _open_endpoint_circuit(url)
            return None

        if response.status_code >= 500:
            _thread_local.last_http_error = f"http_{response.status_code}"
            if route_proxy:
                continue
            _open_endpoint_circuit(url)
            return None
        ...
        if route_proxy:
            _proxy_pool.report_success(_proxy_url_of(route_proxy), elapsed_ms)
        _thread_local.last_http_error = ""
        return response
    _open_endpoint_circuit(url)
    _thread_local.last_http_error = "transport_exhausted"
    return None
...
def get_text_result(
    ...
    if response is None:
        return HttpTextResult(None, None, getattr(_thread_local, "last_http_error", "transport_or_rejected"))
...
def get_text(
    ...
    return get_text_result(
        ...
        max_retries=max_retries, use_proxy=use_proxy, budget_phase=budget_phase,
    ).text
