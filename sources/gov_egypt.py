...
def _fetch_egypt_linkedin_companies():
    ...
    for company_name, slug in LINKEDIN_EG_COMPANIES:
        if time.time() - t0 > budget:
            log.info("gov_egypt/companies: budget hit � stopping early")
            break
...
def _fetch_linkedin_by_governorate():
    ...
    for gov in TOP_TECH_HUBS:
        for kw in SECURITY_KEYWORDS:
            if time.time() - t0 > budget:
                log.info("gov_egypt/governorates: budget hit � stopping early")
                break
            params = (
...
def fetch_gov_egypt():
    ...
    for fetcher in [_fetch_egypt_linkedin_companies, _fetch_linkedin_by_governorate]:
        if time.time() - t0 > LINKEDIN_SOURCE_BUDGET_SECONDS:
            log.info("gov_egypt: 60s total budget hit � stopping remaining sub-sources")
            break
        try:
