...
def fetch_linkedin() -> list[Job]:
    ...
    V22: added session warm-up, smarter batching, longer delays between searches.
    """
    import random

    # ── Warm up the LinkedIn session with a guest homepage visit ──
    # This seeds cookies like a real browser would before hitting the API.
    try:
        warmup_url = "https://www.linkedin.com/jobs/search/?keywords=cybersecurity&location=Egypt"
        get_text(warmup_url)
        log.info("LinkedIn: session warm-up OK")
        time.sleep(random.uniform(3.0, 6.0))
    except Exception:
        log.debug("LinkedIn: warm-up request skipped")
    ...
    # Limit to a reasonable batch per run to avoid triggering blocks
    MAX_SEARCHES = min(len(SEARCHES), 50)
    searches_to_run = SEARCHES[:MAX_SEARCHES]
    ...
    for i, search in enumerate(searches_to_run):
        ...
        if consecutive_failures >= MAX_CONSECUTIVE:
            log.warning(f"LinkedIn: {consecutive_failures} consecutive failures — waiting 60s.")
            time.sleep(60)
            consecutive_failures = 0
        ...
        from sources.http_utils import _linkedin_session
        ...
        if not html:
            ...
            total_failures += 1
            wait = min(15 * consecutive_failures, 90)
        ...
        # Longer pause every 10 searches to mimic human browsing
        if (i + 1) % 10 == 0:
            pause = random.uniform(15.0, 30.0)
            log.info(f"LinkedIn: taking a {pause:.0f}s break after {i+1} searches...")
            time.sleep(pause)
        else:
