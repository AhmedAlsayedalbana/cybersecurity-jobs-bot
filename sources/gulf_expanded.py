...
def _fetch_linkedin_gulf_expanded():
    ...
    for company_name, slug in LINKEDIN_GULF_EXPANDED:
        if _t.time() - t0 > budget:
            log.info(f"gulf_expanded: 6-min budget hit after {len(jobs)} jobs � stopping early")
            break
    ...
    for loc in GULF_INTERN_LOCATIONS:
        for kw in GULF_INTERN_KEYWORDS[:3]:
            params = (
                ...
                "&start=0&count=10&f_TPR=r172800"
...
def _fetch_naukrigulf():
    ...
    for q in queries:
        ...
        # NaukriGulf consistently times out — cap at 8s per query to avoid 100s+ runs
        html = get_text(url, headers=_H, timeout=8)
        if not html:
            continue
        for block in re.findall(
            r'<script[^>]+type="application/ld\+json"[^>]*>(.*?)</script>',
...
def fetch_gulf_expanded():
    ...
    for fetcher in [
        ...
        if time.time() - t0 > LINKEDIN_SOURCE_BUDGET_SECONDS:
            log.info("gulf_expanded: 60s total budget hit � stopping remaining sub-sources")
            break
        try:
