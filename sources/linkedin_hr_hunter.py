...
MAX_BUDGET_SECS = LINKEDIN_SOURCE_BUDGET_SECONDS
...
def fetch_linkedin_hr_hunter() -> list[Job]:
    ...
    for s in _SEARCHES:
        if time.time() - start > MAX_BUDGET_SECS:
            log.info("HR Hunter: budget exhausted early")
            break

        params = {**s, "start": "0", "count": "10", "f_TPR": FRESH_TPR}
        html = linkedin_get_text(_BASE, params=params)
