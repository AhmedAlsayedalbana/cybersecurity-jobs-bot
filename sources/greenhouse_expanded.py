...
from run_budget import cap_source_timeout, source_remaining
...
def _run_batch(boards: list[BoardEntry], label: str, *, budget_sec: float = 90.0) -> list[Job]:
    ...
    for entry in boards:
        if time.time() - batch_start > budget_sec or source_remaining() <= 0:
...
def fetch_greenhouse_expanded() -> list[Job]:
    ...
    for label, boards, budget in batches:
        if time.time() - _start > SOURCE_TIMEOUT or source_remaining() <= 0:
