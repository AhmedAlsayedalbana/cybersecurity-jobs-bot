"""
Freelance Platforms — V12 (Zero-Warning Edition)

REMOVED (dead — caused all warnings):
  ❌ PeoplePerHour — 0 results always (scrape blocked)
  ❌ Guru.com RSS — 404 always
  ❌ Freelancer.com — blocked

CONFIRMED WORKING:
  ✅ Mostaql (مستقل) — Correct RSS URL (Arab freelance #1)
  ✅ Khamsat (خمسات) — JSON-LD scrape
  ✅ Truelancer — RSS feeds working
  ✅ WorkInSecurity.co.uk — UK cybersecurity-specific board RSS
  ✅ Remoteok API — security-tagged freelance/contract jobs
"""
...
# ── 1. Mostaql (مستقل) — Arab Freelance #1 ──────────────────
MOSTAQL_QUERIES = [
...
# ── 2. Khamsat (خمسات) — Arab Micro-Jobs ────────────────────
def _fetch_khamsat():
    jobs = []
    seen = set()
...
# ── 4. WorkInSecurity.co.uk — UK Cybersecurity specific ──────
def _fetch_workinsecurity():
    """WorkInSecurity — UK-focused cybersecurity job board with RSS."""
    jobs = []
    seen = set()
    feeds = [
...
def fetch_freelance():
    """Aggregate freelance platforms — zero-warning edition."""
    jobs = []
