...
CONFIRMED WORKING:
   Mostaql () � Arab freelance #1, RSS working
   Khamsat () � JSON-LD scrape working
   Truelancer � RSS feeds working
   WorkInSecurity � DNS failure (REMOVED)
"""
...
def _fetch_mustaqil():
    jobs = []
    seen = set()
    for q in ["cybersecurity", "security", "penetration", " ", " "]:
...
def _fetch_truelancer():
    ...
    feeds = [
        ...
        # v53: Additional Truelancer security feeds
        "https://www.truelancer.com/freelance-information-security-jobs?format=rss",
        "https://www.truelancer.com/freelance-penetration-testing-jobs?format=rss",
        "https://www.truelancer.com/freelance-security-audit-jobs?format=rss",
    ]
    for url in feeds:
        xml = get_text(url, headers=_H)
        if not xml:
