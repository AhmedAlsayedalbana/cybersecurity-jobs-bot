...
def fetch_jsearch_enhanced() -> list[Job] | SourceResult:
    ...
    api_key = getattr(config, "RAPIDAPI_KEY", "")
    if not api_key:
        # This connector is intentionally optional.  Reporting the missing
        # credential structurally keeps it out of the failure/quarantine path
        # and makes the source's zero result unambiguous in the main log.
        log.info("jsearch_enhanced: RAPIDAPI_KEY not set — not configured.")
        return SourceResult(
            status="not_configured",
            transport="none",
            error_code="missing_rapidapi_key",
        )
