...
class TestV66HrBackendParkCap:
    ...
    def test_cse_is_exempt_from_run_parking(self):
        for _ in range(hr._BACKEND_PARK_STREAK + 2):
            hr._mark_backend_empty("google_cse")
        assert "google_cse" not in hr._backend_parked, (
            "CSE's bounded backoff already paces it; it must not be parked")
