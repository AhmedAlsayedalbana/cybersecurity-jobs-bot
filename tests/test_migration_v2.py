...
class TestPoolBuilder(unittest.TestCase):
    ...
    def test_min_pool_size_relaxes_linkedin_cap_as_last_resort(self):
        # Merged behaviour: the strict 60/40 cap still governs a normal run,
        # but when there is genuinely no non-LinkedIn/secondary supply this
        # cycle, the last-resort relaxation (merged from official-sources)
        # fills up to MIN_POOL_SIZE instead of leaving the pool empty.
        import config
        from intelligence.pool_builder import build_final_pool
