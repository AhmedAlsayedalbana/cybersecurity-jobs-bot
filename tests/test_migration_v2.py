...
class TestGeoClassification(unittest.TestCase):
    ...
    def test_ksa_location(self):
        self.assertEqual(self._geo(location="Riyadh, Saudi Arabia"), "arab")

    def test_uae_location(self):
        self.assertEqual(self._geo(location="Dubai, UAE"), "arab")

    def test_non_gulf_arab_country_is_eligible(self):
        self.assertEqual(self._geo(location="Amman, Jordan"), "arab")
...
class TestDedupeHelpers(unittest.TestCase):
    ...
    def test_same_title_company_with_different_urls_is_not_a_duplicate(self):
        from intelligence.dedupe import are_duplicate_jobs
        a = _job(url="https://jobs.example.com/100", title="Senior SOC Analyst", company="Acme Corp", location="Cairo, Egypt")
        b = _job(url="https://jobs.example.com/200", title="Senior SOC Analyst", company="Acme", location="Cairo")
        self.assertFalse(are_duplicate_jobs(a, b))

    def test_no_duplicate_different_roles(self):
        from intelligence.dedupe import are_duplicate_jobs
        a = _job(url="https://jobs.example.com/300", title="Penetration Tester", company="Sec Co", location="Dubai")
        b = _job(url="https://jobs.example.com/400", title="GRC Analyst", company="Sec Co", location="Dubai")
        self.assertFalse(are_duplicate_jobs(a, b))
