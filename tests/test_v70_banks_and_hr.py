...
def test_hr_query_plan_runs_without_any_api_key_because_jina_index_is_always_usable():
    """v74: the HR plan must stay alive even when serpapi/bing are all
...
def test_hr_clears_stale_serpapi_failure_state_on_healthy_key():
    """A healthy SerpAPI key at startup must clear any stale failure state
    (park) left over from previous runs, so the
    ...
    hps._backend_cooldown_until["serpapi"] = time.time() + 60.0

    with (
        mock.patch.object(hps, "SERPAPI_KEY", "sp-key"),
        mock.patch.object(hps, "get_json", return_value={"organic_results": []}),
    ...
    assert "serpapi" not in hps._backend_cooldown_until
...
def test_hr_query_plan_still_runs_when_at_least_one_backend_can_answer():
    ...
    temporarily cooled serpapi with healthy bing keeps the plan alive."""
    from sources import linkedin_hr_posts_scraper as hps
    ...
    with (
        ...
        # serpapi in backoff (but not parked) +
        # healthy bing → plan runs.
        hps._backend_cooldown_until["serpapi"] = time.time() + 60.0
        jobs = hps.fetch_linkedin_hr_posts_scraper(budget_seconds=2)
