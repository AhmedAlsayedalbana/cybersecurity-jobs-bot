...
class BotLogicTests(unittest.TestCase):
    ...
    def test_every_arab_country_routes_to_arab_jobs_not_only_gulf(self):
        arab_locations = [
            "Riyadh, Saudi Arabia", "Dubai, United Arab Emirates", "Doha, Qatar",
            "Kuwait City, Kuwait", "Manama, Bahrain", "Muscat, Oman",
            "Amman, Jordan", "Beirut, Lebanon", "Damascus, Syria", "Baghdad, Iraq",
            "Ramallah, Palestine", "Sanaa, Yemen", "Tripoli, Libya", "Tunis, Tunisia",
            "Algiers, Algeria", "Rabat, Morocco", "Nouakchott, Mauritania",
            "Khartoum, Sudan", "Mogadishu, Somalia", "Djibouti", "Moroni, Comoros",
        ]
        for index, location in enumerate(arab_locations):
            with self.subTest(location=location):
                job = Job(
                    title="SOC Analyst",
                    company="ArabSec",
                    location=location,
                    url=f"https://example.com/arab-soc-{index}",
    ...
    def test_stale_edge_uses_configured_age_limit(self):
        age_limit = config.MAX_JOB_AGE_HOURS
        fresh = Job(
            title="SOC Analyst",
            company="Acme",
            location="Cairo, Egypt",
            url="https://example.com/fresh-age-limit",
            source="linkedin_unified",
            posted_date=datetime.now() - timedelta(hours=age_limit - 1),
        )
        stale = Job(
            title="SOC Analyst",
            company="Acme",
            location="Cairo, Egypt",
            url="https://example.com/stale-age-limit",
            source="linkedin_unified",
            posted_date=datetime.now() - timedelta(hours=age_limit),
        )
        self.assertFalse(_is_stale_job(fresh))
        self.assertTrue(_is_stale_job(stale))
        self.assertTrue(is_recent_enough(fresh, max_age_hours=age_limit)[0])
        self.assertFalse(is_recent_enough(stale, max_age_hours=age_limit)[0])
    ...
    def test_netsuite_administrator_is_rejected_as_non_cyber(self):
        job = Job(
            title="NetSuite Administrator",
            company="Acme",
            location="Cairo, Egypt",
            url="https://example.com/netsuite-admin",
            source="linkedin_unified",
            description="Administer ERP users, workflows, reports, and integrations.",
        )
        self.assertFalse(is_cybersec_job(job))
    ...
    def test_pool_orders_egypt_then_arab_region_then_remote(self):
        old_max = config.MAX_JOBS_PER_RUN
        old_threshold = config.SCORE_THRESHOLD
        try:
