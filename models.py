...
Geo rules:
  - Physical / hybrid: Egypt or any Arab country only
  - Explicit remote: worldwide
  - Unknown or any other physical location: reject

The query that found a job is never accepted as location evidence.  The
vacancy's explicit location controls geographic delivery.
"""
...
# Delivery must use this module directly.  ``job_intelligence.classify_geo``
# intentionally accepts discovery/query hints for telemetry, whereas this
# function rejects a physical listing with an unknown location.
...
class Job:
    ...
    source_key: str = ""
    # v55 provenance is populated by the public marketplace connectors.  It is
    # deliberately part of the model so strict filtering can reject scraped
    # page chrome and seller-service listings deterministically.
    extraction_method: str = ""
    provenance_hash: str = ""
    # geo_hint: discovery metadata derived from the source query.  It may be
    # used only when a source did not supply a real job location; it must never
    # override an explicit location printed on the vacancy itself.
    geo_hint: str = ""
    cyber_verdict: str = ""
...
class CyberVerdict(str, Enum):
    CONFIRMED = "CYBER_CONFIRMED"
    LIKELY = "CYBER_LIKELY"
    NON_CYBER = "NON_CYBER"
...
class FilterDecision:
    accept: bool
    reason_code: str
    verdict: CyberVerdict = CyberVerdict.NON_CYBER
...
SECURITY_TITLE_PATTERNS = [
    ...
    "security researcher", "security operations", "security operation engineer", "security auditor",
    ...
    "cloud security", "network security engineer", "network security backoffice", "network security back office",
    ...
    "sailpoint", "privileged access", "access management", "ping identity", "okta",
...
CORE_ROLES = [
    "soc analyst", "soc engineer", "security operations engineer", "security operation engineer",
    ...
    "iam engineer", "identity access", "access management", "ping identity", "okta", "pki engineer", "cryptograph",
...
# This is deliberately a *candidate* gate, not an acceptance rule.  It keeps
# the expensive ML triage away from thousands of obviously unrelated rows
# while preserving every realistic cyber pathway for the strict classifier
# below.  Vague infrastructure words ("cloud", "python", "aws") are absent
# on purpose: they caused the exact false positives reported in delivery.
FAST_CYBER_CANDIDATE_PATTERNS = (
    "security", "cyber", "cybersecurity", "cyber security", "infosec",
    "information security", "soc", "siem", "threat", "incident response",
    "dfir", "forensic", "malware", "pentest", "penetration", "red team",
    "blue team", "ethical hacker", "appsec", "application security",
    "devsecops", "network security", "firewall", "zero trust", "sase",
    "ztna", "grc", "iso 27001", "pci dss", "gdpr", "privacy",
    "compliance", "governance", "risk", "auditor", "data protection",
    "data loss prevention", "dlp", "ciso", "iam", "identity access",
    "identity and access", "okta", "ping identity", "privileged access",
    "pam", "pki", "cryptograph", "vulnerability", "cve", "edr", "xdr",
    "cspm", "cnapp", "cwpp", "cloud security", "aws security",
    "azure security", "gcp security", "kubernetes security", "container security",
    "trust and safety", "trust & safety", "abuse prevention", "content integrity",
    "online safety",
    "أمن سيبراني", "الأمن السيبراني", "أمن معلومات", "اختبار اختراق",
    "محلل أمن", "مهندس أمن",
)
...
def _is_fast_cyber_candidate(job: "Job") -> bool:
    """Cheap conservative pre-gate before local ML inference.

    A row with no cyber/security/domain vocabulary cannot produce
    publish-grade evidence later in the pipeline.  Rejecting it here prevents
    high-volume general ATS feeds from consuming the delivery window.
    """
    text = " ".join([
        job.title or "",
        _flatten_tags(job.tags),
        (job.description or "")[:700],
    ]).lower()
    return any(_word_match(pattern, text) for pattern in FAST_CYBER_CANDIDATE_PATTERNS)


def is_cybersec_job(job: "Job") -> bool:
    # The LLM may be used as a later veto, never as an automatic acceptance
    # path.  Deterministic evidence remains the publish gate.
    decision = classify_cyber_intent(job, use_llm=False)
    ...
    if not decision.accept:
        # Per-job rejection logs can be thousands of lines and hide the run
        # metrics that operators need.  Keep the evidence available at DEBUG.
        logger.debug("Job excluded (%s): %s", decision.reason_code, job.title)
    return decision.accept


def passes_geo_filter(job: "Job") -> bool:
    """
    Physical / hybrid roles are accepted only in Egypt or an Arab country.
    Explicitly remote roles are accepted worldwide.  Query location and source
...
_STRONG_ML_RESCUE_ANCHORS = {
    ...
    "security engineer", "security analyst", "security architect", "security operations", "security operation engineer",
    "security researcher", "phishing analyst", "security program",
    "soc", "siem", "dfir", "incident response", "threat intelligence", "threat hunting",
    "threat intelligence researcher", "supply chain security",
    ...
    "iam", "identity and access", "identity access", "access management", "ping identity", "okta", "zero trust",
...
def classify_jobs(jobs: list["Job"]) -> tuple[list["Job"], list["Job"]]:
    """
    Local hybrid filter:
    ...
    for job in jobs:
        ...
        if config.STRICT_PUBLIC_ONLY:
            content_type = (getattr(job, "content_type", "") or "").lower()
            if content_type not in {"job_listing", "client_project", "hr_post"}:
                decision.reason_code = "reject_non_job_opportunity_type"
                setattr(job, "filter_reason", decision.reason_code)
                rejected.append(job)
                continue
            ...
            if source_key in public_target_sources and not (
                getattr(job, "extraction_method", "") and getattr(job, "provenance_hash", "")
            ):
                decision.reason_code = "reject_missing_public_provenance"
                setattr(job, "filter_reason", decision.reason_code)
                rejected.append(job)
                continue
        ...
        # Run the ML/intent stack only for a conservative cyber candidate.
        # This must live after canonical URL/public-provenance validation so
        # rejected telemetry remains truthful for every fetched record.
        if not _is_fast_cyber_candidate(job):
            decision.reason_code = "reject_fast_no_cyber_signal"
            setattr(job, "filter_reason", decision.reason_code)
            rejected.append(job)
            fast_rejected += 1
            continue
        ...
        if keyword_result:
            ...
            anchor_hits = _count_cyber_anchor_hits(f"{job.title} {job.description} {_flatten_tags(job.tags)}")
            is_confirmed = (
                has_strong_cyber_anchor(job)
                or triage_label == "high_confidence"
                or (ml_prob >= 0.82 and anchor_hits >= 1)
            )
            if is_confirmed:
                decision = FilterDecision(True, intent_reason, CyberVerdict.CONFIRMED, ml_prob)
            elif ml_prob >= config.CYBER_LIKELY_MIN_PROB and anchor_hits >= 1:
                decision = FilterDecision(True, "likely_keyword_context", CyberVerdict.LIKELY, ml_prob)
            else:
                decision = FilterDecision(False, "reject_low_confidence_keyword", CyberVerdict.NON_CYBER, ml_prob)
                setattr(job, "filter_reason", decision.reason_code)
                rejected.append(job)
                continue
            setattr(job, "filter_reason", decision.reason_code)
            job.cyber_verdict = decision.verdict.value
            job.cyber_probability = ml_prob
            if isinstance(job.tags, list):
                job.tags.append(f"cyber_verdict:{decision.verdict.value.lower()}")
            accepted.append(job)
            continue

        # A guarded ML candidate is useful review/sending inventory rather
        # than an automatic discard.  Hard rejects and missing anchors remain
        # non-cyber to keep the likely lane precise.
        rescue_ok, rescue_reason = _ml_rescue_guard(job, ml_prob)
        if triage_label != "hard_reject" and ml_prob >= config.CYBER_LIKELY_MIN_PROB and rescue_ok:
            decision = FilterDecision(True, f"likely_ml:{rescue_reason}", CyberVerdict.LIKELY, ml_prob)
            setattr(job, "filter_reason", decision.reason_code)
            job.cyber_verdict = decision.verdict.value
            job.cyber_probability = ml_prob
            if isinstance(job.tags, list):
    ...
    # The pipeline is intentionally ordered: cyber verdict first, then the
    # physical/remote location rule. A valid cyber job outside the permitted
    # geography is not mislabeled as NON_CYBER; it is a location rejection.
    cyber_candidates = accepted
    accepted = []
    for job in cyber_candidates:
        if not passes_geo_filter(job):
            setattr(job, "filter_reason", "reject_geo_filter")
            rejected.append(job)
            continue
        ...
        if not recent_ok:
            setattr(job, "filter_reason", recent_reason)
            rejected.append(job)
            continue
        accepted.append(job)

    for job in accepted:
        if not job.cyber_verdict:
            job.cyber_verdict = CyberVerdict.CONFIRMED.value
    for job in rejected:
        # Cyber candidates rejected only for location/recency retain their
        # verdict for stage-level audit metrics. They are still not returned to
        # the delivery pipeline.
        if not job.cyber_verdict:
            job.cyber_verdict = CyberVerdict.NON_CYBER.value
        if job.cyber_probability is None:
            job.cyber_probability = 0.0
    rescued = sum(1 for j in accepted if str(getattr(j, "filter_reason", "")).startswith("likely_ml"))
    verdict_confirmed = sum(j.cyber_verdict == CyberVerdict.CONFIRMED.value for j in jobs)
    verdict_likely = sum(j.cyber_verdict == CyberVerdict.LIKELY.value for j in jobs)
    verdict_non_cyber = sum(j.cyber_verdict == CyberVerdict.NON_CYBER.value for j in jobs)
    post_verdict_rejected = sum(
        1 for j in rejected
        if j.cyber_verdict in {CyberVerdict.CONFIRMED.value, CyberVerdict.LIKELY.value}
    )
    logger.info(
        f"Cyber verdict stage: confirmed={verdict_confirmed} likely={verdict_likely} "
        f"non_cyber={verdict_non_cyber}; publishable_after_geo_and_recency={len(accepted)} "
        f"post_verdict_rejected={post_verdict_rejected}"
        + (f" (ML rescued {rescued} anchored borderline titles)" if rescued > 0 else "")
    )
    if fast_rejected:
        logger.info(
            "Fast cyber pre-gate: rejected=%d before ML/intent inference.", fast_rejected
        )
    return accepted, rejected


def filter_jobs(jobs: list["Job"]) -> list["Job"]:
    """Backward-compatible publishing candidate view of ``classify_jobs``."""
    accepted, _ = classify_jobs(jobs)
    return accepted
