...
from classifier import classify_location  # Uses intelligence/ module internally
...
WEIGHTS = {
    ...
    "egypt":         8,
    "arab":          6,
    "gulf":          6,  # legacy readers; new scoring uses "arab"
    "remote":        5,
    "global":        1,
    "hybrid_bonus":  1,
...
def score_job(job: Job) -> Tuple[int, List[str]]:
    ...
    try:
        ...
        if "egypt" in loc_lower or "cairo" in loc_lower:
            loc_type = "egypt"
        elif any(x in loc_lower for x in config.ARAB_PATTERNS):
            loc_type = "arab"
        else:
            loc_type = "global"
    ...
    if loc_type == "egypt":
        ...
        score += WEIGHTS["arab"]
        reasons.append(f"+{WEIGHTS['arab']} Arab region")
    elif is_remote:
    ...
    # Hybrid only when Egypt/Arab region + confirmed remote option
    if loc_type in ("egypt", "arab") and is_remote:
    ...
    elif src in ("linkedin_li_at", "linkedin_auth"):
        # LinkedIn Authenticated � highest quality, no ban risk
        score += WEIGHTS["src_li_reg"] + 1
        reasons.append(f"+{WEIGHTS['src_li_reg'] + 1} linkedin authenticated")
    elif src in ("linkedin", "linkedin_hiring", "linkedin_posts", "linkedin_hr",
                 "linkedin_hr_post", "linkedin_hr_hunter", "linkedin_egypt_arabic"):
...
def sort_by_location_priority(jobs_with_scores: list) -> list:
    """Sort (job, score) pairs: Egypt  Arab region  Rest, then score."""
    def priority(item):
        ...
        try:
            ...
            loc = "global"
        return (0 if loc == "egypt" else 1 if loc == "arab" else 2, -score)
