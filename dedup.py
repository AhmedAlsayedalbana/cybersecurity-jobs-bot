...
def deduplicate(jobs: list, seen_dict: dict) -> list:
    ...
    for job in jobs:
        ...
        # Layer 1: exact ID + URL dedup
        if uid in seen_dict or uid in batch_ids:
            continue
        if url_id and (url_id in seen_dict or url_id in batch_ids):
            continue
        # v53: URL-normalized dedup — same job URL from different source keys
        job_url = getattr(job, "url", "") or ""
        url_norm = re.sub(r"[?#].*$", "", job_url.lower().rstrip("/"))  # strip query/fragment
        if url_norm and url_norm in seen_dict:
            log.debug("[dedup] URL-norm dupe: %s", job.title)
            continue
        ...
        if url_norm:                        # v53: track normalized URL within batch
            batch_ids.add(url_norm)
        batch_fps.append(fp)
        new_jobs.append(job)
