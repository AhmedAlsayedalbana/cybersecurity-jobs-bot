...
async def _fetch_one(spec: SourceSpec, stats: dict, db: JobsDB) -> list:
    ...
    try:
        if asyncio.iscoroutinefunction(fetcher):
            ...
            raw_result = await asyncio.to_thread(fetcher)
        ...
        healthy_success = result.status in {"success", "empty", "no_public_client_feed"}
        ...
        if result.status == "empty":
            # An available public API can truthfully have no fresh security
            # roles at this instant.  Log it distinctly from a broken/blocked
            # source so the health report never turns an empty result into a
            # misleading "0 jobs" failure.
            log.info(f"    {name}: no current matching roles [empty/{result.transport}] ({elapsed}ms)")
        else:
            log.info(f"    {name}: {len(jobs)} jobs [{result.status}/{result.transport}] ({elapsed}ms)")
        return jobs
...
async def fetch_all_async(stats: dict, db: JobsDB) -> list:
    ...
    for spec, batch in zip(run_specs, results):
        ...
        for job in batch:
            ...
            if int(getattr(job, "origin_priority", 999) or 999) == 999:
...
def main():
    ...
    try:
        ...
        if is_seed:
            if config.DRY_RUN:
                log.info(f"DRY_RUN seed: would mark {len(new_jobs)} jobs seen")
            else:
            ...
            # 6. Send to Telegram
            if final_pool and not config.DRY_RUN:
            ...
            elif final_pool and config.DRY_RUN:
                log.info("DRY_RUN enabled — Telegram delivery and seen-state persistence skipped.")
                sent_records = []
            else:
