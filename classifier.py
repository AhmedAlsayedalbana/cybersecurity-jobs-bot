...
classify_location: egypt / arab / global
...
def _location_is_ambiguous(loc: str) -> bool:
    ...
    remote/flexible signals, i.e. it does NOT contain any Egypt or Arab-region geo token.
    """
    if not loc:
        return True
    ...
    if any(x in loc for x in config.EGYPT_PATTERNS):
        return False
    if any(x in loc for x in config.ARAB_PATTERNS):
        return False
...
def classify_location(job: Job) -> str:
    """
    Classify job location into 'egypt', 'arab', or 'global'.

    Priority order:
    1. job.location field (AUTHORITATIVE if not ambiguous).
       - Matches Egypt pattern  'egypt'  (no further checks)
        - Matches Arab-region pattern   'arab'   (no further checks)
