...
class JobsDB:
        ...
        with self._conn() as con:
            con.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS review_samples (
                    ...
                    reason TEXT,
                    description_short TEXT,
                    population_size REAL NOT NULL DEFAULT 1,
                    label INTEGER,
                    reviewer_id INTEGER,
                    reviewed_at TEXT,
                    created_at TEXT NOT NULL
                );
                CREATE INDEX IF NOT EXISTS idx_review_run ON review_samples(run_id, label);
                CREATE INDEX IF NOT EXISTS idx_review_dedup ON review_samples(dedup_key);
        ...
        try:
            ...
            con.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS telegram_delivery_outbox (
                    ...
                    next_retry_at TEXT, sent_at TEXT
                );
                CREATE INDEX IF NOT EXISTS idx_delivery_outbox_due
                    ON telegram_delivery_outbox(status, next_retry_at);
            """)
            ...
            # ``reserved`` means only that a payload was prepared.  A durable
            # ``sent_at`` timestamp is the sole proof that a channel/job pair
            # was delivered.  Existing v59 ``sent`` rows predate this column,
            # so preserve their historical success using ``updated_at``.
            try:
                con.execute("ALTER TABLE telegram_delivery_outbox ADD COLUMN sent_at TEXT")
                con.commit()
                log.info("[DB] Migrated: added sent_at to Telegram delivery outbox.")
            except sqlite3.OperationalError:
                pass
            # Also covers databases created after the schema change.
            con.execute(
                "UPDATE telegram_delivery_outbox "
                "SET sent_at=updated_at WHERE status='sent' AND sent_at IS NULL"
            )
            con.commit()
    ...
    def mark_telegram_delivery(self, delivery_key: str, *, status: str,
        ...
        with self._conn() as con:
            con.execute(
                ...
                WHERE delivery_key=?""",
                (
                    status, (error or "")[:500], now.isoformat(), next_retry,
                    status, now.isoformat(), delivery_key,
                ),
            )
    ...
    def get_due_safe_delivery_retries(self, limit: int = 25) -> list[dict]:
        ...
        with self._conn() as con:
            rows = con.execute(
                ...
                WHERE status='retry_429' AND next_retry_at <= ? AND attempts < 2
    ...
    def create_review_sample(
        self, *, token: str, run_id: str, dedup_key: str, predicted_verdict: str,
        score_band: str, stratum_json: str, title: str, company: str, location: str,
        source: str, reason: str, description_short: str, population_size: int,
    ) -> None:
        with self._conn() as con:
            con.execute(
                """INSERT OR IGNORE INTO review_samples(
                    token, run_id, dedup_key, predicted_verdict, score_band,
                    stratum_json, title, company, location, source, reason,
                    description_short, population_size, created_at
                ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (token, run_id, dedup_key, predicted_verdict, score_band, stratum_json,
                 title, company, location, source, reason, description_short[:800],
                 max(1.0, float(population_size)), datetime.now().isoformat()),
            )

    def get_telegram_update_offset(self) -> int:
        with self._conn() as con:
            row = con.execute(
                "SELECT value FROM telegram_update_state WHERE state_key='review_offset'"
            ).fetchone()
        try:
        ...
        with self._conn() as con:
            con.execute(
                """INSERT INTO telegram_update_state(state_key, value, updated_at)
                   VALUES('review_offset', ?, ?)
                   ON CONFLICT(state_key) DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at""",
                (str(int(offset)), datetime.now().isoformat()),
            )

    def record_review_label(self, token: str, label: bool, reviewer_id: int) -> bool:
        """First trusted label wins; duplicate callbacks are idempotent."""
        with self._conn() as con:
            result = con.execute(
                """UPDATE review_samples
                   SET label=?, reviewer_id=?, reviewed_at=?
                   WHERE token=? AND label IS NULL""",
                (1 if label else 0, int(reviewer_id), datetime.now().isoformat(), token),
            )
            return bool(result.rowcount)

    def get_review_sample(self, token: str) -> dict | None:
        with self._conn() as con:
            row = con.execute("SELECT * FROM review_samples WHERE token=?", (token,)).fetchone()
        return dict(row) if row else None

    def get_review_metrics(self, run_id: str | None = None) -> dict:
        with self._conn() as con:
            if run_id:
                rows = con.execute(
                    """SELECT predicted_verdict, label, population_size
                       FROM review_samples WHERE run_id=?""", (run_id,)
                ).fetchall()
            else:
                rows = con.execute(
                    """SELECT predicted_verdict, label, population_size
        ...
        total = len(rows)
        reviewed = [row for row in rows if row["label"] is not None]
        weighted = {"tp": 0.0, "fp": 0.0, "tn": 0.0, "fn": 0.0}
        for row in reviewed:
            predicted_positive = row["predicted_verdict"] != "NON_CYBER"
            actual_positive = bool(row["label"])
            weight = float(row["population_size"] or 1)
            key = "tp" if predicted_positive and actual_positive else (
                "fp" if predicted_positive else ("fn" if actual_positive else "tn")
            )
            weighted[key] += weight
        precision_den = weighted["tp"] + weighted["fp"]
        recall_den = weighted["tp"] + weighted["fn"]
        return {
            "scope": "run" if run_id else "cumulative",
            "samples": total, "reviewed": len(reviewed),
            "coverage": round(len(reviewed) / max(1, total), 4),
            "tp": weighted["tp"], "fp": weighted["fp"],
            "tn": weighted["tn"], "fn": weighted["fn"],
            "precision": (weighted["tp"] / precision_den) if precision_den else None,
            "recall": (weighted["tp"] / recall_den) if recall_den else None,
        }

    def save_run_metrics(self, run_id: str, metrics_json: str) -> None:
        with self._conn() as con:
            con.execute(
                """INSERT INTO run_metrics(run_id, created_at, metrics_json) VALUES(?,?,?)
                   ON CONFLICT(run_id) DO UPDATE SET metrics_json=excluded.metrics_json""",
                (run_id, datetime.now().isoformat(), metrics_json),
            )
