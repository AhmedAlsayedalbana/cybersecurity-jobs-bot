...
class JobsDB:
    ...
    def cleanup_old(self, days: int = None):
        ...
        with self._conn() as con:
            ...
            log.info(f"[DB] Cleanup: removed {deleted_unsent} unseen + {result2.rowcount} old sent jobs.")
        ...
        with self._conn() as con:
            con.execute(
                ...
                INSERT INTO job_training_samples(
                    ...
                    description_short, accepted, reason, collected_at
                ) VALUES(?,?,?,?,?,?,?,?,?,?)
                """,
                (
                    dedup_key or "",
    ...
    def get_training_samples(self, *, days: int = 60, limit: int = 5000) -> list[dict]:
