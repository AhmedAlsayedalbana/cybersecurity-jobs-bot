...
Runs on GitHub Actions every **5 hours** (cron: `0 */5 * * *`).
...
| # | Change | Details |
|---|--------|---------|
| 1 | ⏱️ Schedule | Changed from every 2h → **every 5 hours** (`0 */5 * * *`) |
| 2 | 📤 Channel limit | Raised `MAX_JOBS_PER_CHANNEL` from 5 → **10** per run |
| 3 | 🔁 Dedup window | `GLOBAL_DEDUP_HOURS` set to **5h** to match cron interval |
| 4 | ✅ All channels fill | Fallback fill ensures **no channel is left empty** when jobs exist |
| 5 | 🚫 False positive fix | Expanded `PHYSICAL/COMMERCIAL/GENERIC/BUSINESS` hard-reject lists with 40+ new patterns seen in logs |
| 6 | 🚫 Non-cyber domains | Added `reject_non_cyber_security_domain` (food/water/energy security) |
| 7 | 🚫 Generic IT filter | `reject_generic_it_management` for IT Manager/Director with < 2 cyber context hits |
| 8 | 🎭 Fake job filter | Blocks Gilfoyle/Silicon-Valley-generator GitHub issues at source |
| 9 | 🗑️ Garbage titles | Early-exit filter for Cloudflare challenge pages, nav items, URL fragments |
| 10 | 🔬 ML rescue tightened | Raised `context_stack` threshold 3→4; expanded `_ML_RESCUE_TITLE_BLOCKERS` |
| 11 | 🌐 Jina scraper fix | Skips page headings & nav items extracted as fake job titles |
...
| `GOOGLE_CSE_API_KEY` / `GOOGLE_CSE_CX` | Google Custom Search backend for LinkedIn HR posts (primary backend) |
...
| `HR_HIRING_THRESHOLD` | Hiring-signal threshold for HR post acceptance (default: `8`) |
| `HR_CONFIDENCE_THRESHOLD` | Total confidence threshold for HR post acceptance (default: `12`) |
| `ENABLE_SOURCE_LINKEDIN_EGYPT_ARABIC` | Enable Arabic LinkedIn Egypt source (default: `0`) |
| `ENABLE_SOURCE_EGYPT_COMPANIES` | Enable Egypt Companies source (default: `0`) |
| `ENABLE_SOURCE_JSEARCH` | Enable JSearch source gate (still requires `RAPIDAPI_KEY`) |
| `ENABLE_SOURCE_REMOTIVE` | Enable Remotive source (default: `0`) |
| `ENABLE_SOURCE_ARBEITNOW` | Enable Arbeitnow source (default: `0`) |
| `ENABLE_SOURCE_WWR` | Enable WeWorkRemotely source (default: `0`) |
