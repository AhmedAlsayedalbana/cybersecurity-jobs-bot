...
def are_duplicate_jobs(a: Any, b: Any, *, threshold: float = 0.72) -> bool:
    ...
    if url_a and url_b and url_a == url_b:
        title_a = normalize_text(getattr(a, "title", "") or "")
        title_b = normalize_text(getattr(b, "title", "") or "")
        company_a = normalize_text(getattr(a, "company", "") or "")
        company_b = normalize_text(getattr(b, "company", "") or "")
        if title_a == title_b:
            return True
        if company_a and company_b and company_a == company_b:
            return fuzzy_match(title_a, title_b, threshold=0.60)
        return False
