...
def _is_allowed_technical_commercial_role(job: Any) -> bool:
    """Allow only deeply technical cyber presales/consulting roles."""
    title = job_title(job)
    full = job_full_text(job)
    commercial_title = has_any(
        [
    ...
    if not commercial_title:
        return False
    technical_security_title = has_any(
        [
            "cybersecurity architect", "cyber security architect",
            "security architect", "security technical architect",
            "cloud security consultant", "network security consultant",
            "application security consultant", "cybersecurity consultant",
            "xsiam consultant",
        ],
        title,
    )
    if not technical_security_title:
        return False
...
def hard_reject_reason(job: Any) -> str | None:
    ...
    if has_any(COMMERCIAL_HARD_REJECTS, title) and not _is_allowed_technical_commercial_role(job):
...
def is_true_security_internship(job: Any) -> bool:
    title = job_title(job)
    full = job_full_text(job)
    has_entry_signal = bool(ENTRY_RE.search(full))
    ...
    if has_any(["intern", "internship", "trainee", "graduate", "entry level", "entry-level"], title):
        return count_hits(CYBER_CONTEXT_PATTERNS, full) >= 2
    return False
    ...
    if borderline and use_llm:
        ...
        if llm_result is True:
            return CyberIntentDecision(
                True, "accept_llm_borderline", 0.78, ["llm_accept"], borderline=True
            )
