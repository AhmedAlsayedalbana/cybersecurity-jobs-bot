...
def build_final_pool(jobs: list, score_fn: Callable) -> list:
    ...
    def _try_add(job: Any, *, enforce_li_cap: bool = True) -> bool:
        ...
        if enforce_li_cap and is_li:
            projected_len = len(selected) + 1
            projected_li = linkedin_count + 1
            dynamic_cap = max(
                1,
                round(projected_len * config.LINKEDIN_POOL_CAP_RATIO) + 1,
            )
            if projected_li > dynamic_cap:
                return False
        selected.append(job)
