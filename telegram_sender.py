...
def _topic_channel_for_job(job, searchable: str) -> str | None:
    ...
    return classify_intelligence_domain(job)
...
def _drain_retry_queue(db: JobsDB) -> int:
    """Retry only Telegram 429 responses, which are known not to be delivered."""
    sent = 0
    for row in db.get_due_safe_delivery_retries(limit=TELEGRAM_RETRY_DRAIN_LIMIT):
        ...
        if status == 429:
            ...
            # A timeout or 5xx can have reached Telegram after all.  Never
            # auto-repeat it, because duplicate job posts are worse than a
            # recoverable alert for the administrator.
            db.mark_telegram_delivery(
                row["delivery_key"], status="ambiguous",
                error=f"status={status} {err}".strip(),
            )
    return sent
...
def send_jobs(jobs):
    ...
    # LinkedIn: 10, Egyptian: 15-22, public contract feeds: 27-30, others: 31+
    def _source_priority_key(job) -> int:
        return int(getattr(job, "origin_priority", 999) or 999)

    def _is_linkedin_delivery(job) -> bool:
    ...
    # ── Sort: source_priority ASC (LinkedIn first), then score DESC ─────────
    # This ensures LinkedIn jobs appear first within each channel
    jobs_scored = sorted(
        jobs,
        key=lambda j: (_source_priority_key(j), -score_job_int(j))
    )
    ...
    for ch_key in TOPIC_CHANNELS:
        ...
        candidates = []
        for j in jobs_scored:
            jkey = getattr(j, "dedup_key", "") or getattr(j, "url_id", "") or j.unique_id
    ...
    # The pool builder applies the same balance before this point.  Keep a
    # second gate here because channel routing/dedup can otherwise change the
    # mix after selection (one job can route to a geo and a topic thread).
    linkedin_sent = 0
    non_linkedin_sent = 0
    linkedin_ratio = min(1.0, max(0.0, float(getattr(_cfg, "LINKEDIN_POOL_CAP_RATIO", 0.60))))
    deferred_linkedin: list[tuple[str, object]] = []

    def _linkedin_slot_available() -> bool:
        if linkedin_ratio >= 1.0:
            return True
        return (linkedin_sent + 1) <= ((total_sent + 1) * linkedin_ratio + 1e-9)
    ...
    while True:
        ...
        for ch_key in send_order:
            ...
            while channel_cursors[ch_key] < len(queue):
                ...
                # Do not let delivery routing silently turn the selected
                # 60/40 pool into LinkedIn-only Telegram output.  Hold an
                # early LinkedIn item until enough non-LinkedIn items have
                # actually been delivered, then retry it below.
                if _is_linkedin_delivery(job) and not _linkedin_slot_available():
                    deferred_linkedin.append((ch_key, job))
                    continue
    ...
    # The first pass favours non-LinkedIn candidates only until the ratio
    # permits LinkedIn.  Revisit deferred items so their earlier position in a
    # source-priority-sorted queue never makes them disappear unnecessarily.
    while deferred_linkedin and _linkedin_slot_available():
        delivered = False
        remaining: list[tuple[str, object]] = []
        for ch_key, job in deferred_linkedin:
            if channel_summary[ch_key] >= limit:
                continue
            if not _linkedin_slot_available():
                remaining.append((ch_key, job))
                continue
            thread_id = get_topic_thread_id(ch_key)
            if not thread_id:
                continue
            is_topic = ch_key not in GEO_CHANNELS
            lane = "topic" if is_topic else "geo"
            title_key = re.sub(r"\s+", " ", (job.title or "").strip().lower())
            url_id = getattr(job, "url_id", "")
            job_dedup_key = getattr(job, "dedup_key", "") or url_id or job.url or job.unique_id
            if title_key and title_key in channel_titles_sent[ch_key]:
            ...
            if is_topic and job_dedup_key:
                topic_globally_sent.add(job_dedup_key)
            log.info(
                f"   [{ch_key}] {channel_summary[ch_key]}/{limit} ✓ [LI] {job.title[:45]}"
            )
            time.sleep(TELEGRAM_SEND_DELAY)
            delivered = True
        deferred_linkedin = remaining
        if not delivered:
            break
    ...
    if total_sent:
        log.info(
            " Delivered source mix: LinkedIn %d (%.1f%%) | Other %d (%.1f%%)",
            linkedin_sent, (linkedin_sent / total_sent) * 100,
            non_linkedin_sent, (non_linkedin_sent / total_sent) * 100,
        )
    if deferred_linkedin:
        log.info(
            " LinkedIn delivery cap held %d item(s) to preserve the %.0f/%.0f final mix.",
            len(deferred_linkedin), linkedin_ratio * 100, (1 - linkedin_ratio) * 100,
        )
...
def _send_to_topic(message, thread_id=None, db: JobsDB | None = None, channel_key: str = "",
    ...
    if db and delivery_id:
        if not db.reserve_telegram_delivery(
            ...
            log.info("Telegram delivery already reserved/sent for [%s]", channel_key)
            return False
    ...
    if db and delivery_id:
        if status == 429:
            ...
            db.mark_telegram_delivery(
                delivery_id, status="ambiguous", error=f"status={status} {err}".strip(),
            )
            log.error("Telegram delivery is ambiguous for [%s]; it will not auto-retry.", channel_key)
        else:
            db.mark_telegram_delivery(
                delivery_id, status="failed", error=f"status={status} {err}".strip(),
            )
    return False
