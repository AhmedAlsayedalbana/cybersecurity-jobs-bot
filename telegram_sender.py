...
def send_jobs(jobs):
    ...
    - Jobs older than 2 days (48h) are HARD-BLOCKED from sending, regardless of source.
    ...
    # ── 2-day hard stale gate ───────────────────────────────────────────────
    SEND_STALE_HOURS = 48  # jobs older than 48h are never sent
    now = datetime.now()
    ...
    if stale_skipped_total:
        log.info(f" ⏰ Stale gate: skipped {stale_skipped_total} job(s) older than 48h")
